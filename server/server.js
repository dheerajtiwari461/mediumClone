...
import serviceAccountKey from "./mern-blog-c7492-firebase-adminsdk-5i4ya-e5a5f17fb7.json" assert {type: "json"};
...
server.post("/signin", (req,res)=>{
    ...
    .then((user)=>{
        ...
        if(!user.google_auth){
            ...
            return res.status(403).json({"error": "Account was created with google. Please login with google"});
        }
...
server.post("/google-auth", async (req,res)=>{
    ...
    .then(async (decodedUser)=>{
        ...
        picture = picture.replace("s96-c", "384-c");
        ...
        if(user){ // login
            if(!user.google_auth){
                return res.status(403).json({"error": "Please login with email and password"});
            }
